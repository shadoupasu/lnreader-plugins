...
class NovelOkuTR implements Plugin.PluginBase {
    ...
    async getNovelDetails(novelPath: string): Promise<NovelDetails> {
        try {
            ...
            const novel: NovelDetails = {
                ...
                genres: this.extractGenres(loadedCheerio),
                status: loadedCheerio(".post-status .summary-content").text().trim() || "Unknown",
                chapters: [],
            };
            ...
            console.error("Error in getNovelDetails:", error);
            return {
                path: novelPath,
    ...
    private extractGenres(cheerio: any): string {
        try {
            const genres: string[] = [];
